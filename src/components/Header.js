...
const Header = () => {
  ...
  return (
    <header className="sticky sm:top-[22px] bg-white text-slate-700 md:py-4 z-[99] ">
      <div className="container mx-auto flex flex-wrap justify-between items-center p-4 md:p-0">
        <Link href="/">
          <img
            src="/logo.svg"
            alt="Клиника красоты SarBona"
            className="hidden sm:block h-16 w-auto mr-8 ml-6 xl:ml-0"
          />
        </Link>
        ...
        <div className="lg:hidden flex flex-grow justify-center mr-5">
          <button
            onClick={openPopup}
            className="px-2 py-2 sm:px-4 sm:py-4 bg-light-golden text-white rounded hover:bg-dark-golden transition duration-300"
          >
            Записаться на сеанс
          </button>
        </div>
          ...
          <nav className="w-full lg:flex lg:items-center lg:justify-between">
            <ul className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4 w-full">
              <li className="lg:hidden">
                <Link
                  href="#services"
                  className="transition-colors duration-300 hover:text-light-golden flex items-center px-5 py-5 text-xl sm:text-3xl md:text-base"
                >
                  Услуги
                </Link>
              </li>
        ...
        <div className="flex gap-2 md:gap-4 mr-2 md:mr-0">
